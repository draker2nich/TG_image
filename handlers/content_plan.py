...
from states.generation_states import ContentPlanStates
from keyboards.menus import cancel_kb, back_to_menu_kb, main_menu_kb
from services.content_plan_service import content_plan_service, ContentIdea
from services.openai_service import openai_service
from services.google_service import google_service
...
def plan_navigation_kb(page: int, total_pages: int):
    """Клавиатура ТОЛЬКО с навигацией (без перегенерации и генерации контента)"""
    builder = InlineKeyboardBuilder()
    nav = []
    if page > 0:
        nav.append(InlineKeyboardButton(text="⬅️", callback_data=f"plan:page:{page-1}"))
...
def plan_full_kb(page: int, total_pages: int):
    """Полная клавиатура с навигацией, перегенерацией и переходом в меню"""
    builder = InlineKeyboardBuilder()
    nav = []
    if page > 0:
        nav.append(InlineKeyboardButton(text="⬅️", callback_data=f"plan:page:{page-1}"))
    ...
    if nav:
        builder.row(*nav)
    builder.row(InlineKeyboardButton(text="🔄 Перегенерировать план", callback_data="plan:regenerate"))
    builder.row(InlineKeyboardButton(text="🎬 Генерировать контент", callback_data="plan:go_to_menu"))
    return builder.as_markup()
...
async def generate_plan(callback: CallbackQuery, state: FSMContext):
    ...
    try:
        ...
        await state.set_state(ContentPlanStates.viewing_plan)
        await show_content_plan(callback.message, plan, 0, full_controls=True)
    except Exception as e:
        await callback.message.edit_text(f"❌ Ошибка: {e}", reply_markup=back_to_menu_kb())
        await state.clear()


async def show_content_plan(message, plan, page: int = 0, full_controls: bool = True):
    """
    Показывает контент-план с пагинацией.
    
    Args:
        full_controls: True - полные кнопки (навигация + перегенерация + меню)
                      False - только навигация
    """
    ideas = plan.ideas if hasattr(plan, 'ideas') else plan.get("ideas", [])
    if ideas and isinstance(ideas[0], dict):
        ideas = [ContentIdea(**i) for i in ideas]
    ...
    if full_controls:
        keyboard = plan_full_kb(page, total_pages)
    else:
        keyboard = plan_navigation_kb(page, total_pages)
...
async def change_page(callback: CallbackQuery, state: FSMContext):
    ...
    await show_content_plan(callback.message, data.get("content_plan", {}), page, full_controls=True)
    await callback.answer()
...
@router.callback_query(ContentPlanStates.viewing_plan, F.data == "plan:go_to_menu")
async def go_to_menu_from_plan(callback: CallbackQuery, state: FSMContext):
    """
    Переход в главное меню:
    1. Обновляем контент-план - убираем кнопки перегенерации и меню, оставляем только навигацию
    2. Отправляем главное меню новым сообщением
    3. Переводим в состояние "только просмотр"
    """
    data = await state.get_data()
    content_plan = data.get("content_plan", {})
    current_page = data.get("current_page", 0)
    
    # Сохраняем ID сообщения с планом для навигации
    plan_message_id = callback.message.message_id
    
    # Обновляем сообщение с контент-планом - только навигация
    if content_plan:
        await show_content_plan(callback.message, content_plan, current_page, full_controls=False)
    
    # Сохраняем данные для режима просмотра и переводим в новое состояние
    await state.update_data(
        plan_message_id=plan_message_id,
        view_only=True
    )
    await state.set_state(ContentPlanStates.view_only)
...
@router.callback_query(ContentPlanStates.view_only, F.data.startswith("plan:page:"))
async def change_page_view_only(callback: CallbackQuery, state: FSMContext):
    """Навигация по контент-плану в режиме только просмотра"""
    data = await state.get_data()
    content_plan = data.get("content_plan", {})
    ...
    page = int(callback.data.split(":")[2])
    await state.update_data(current_page=page)
    await show_content_plan(callback.message, content_plan, page, full_controls=False)
    await callback.answer()